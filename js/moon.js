...
function showMoonMessage(message, duration = 3000) {
    ...
    function showMoonMessage(message, duration = 3000) {
    // Son de la Lune qui parle
    if (typeof playSFX === 'function') playSFX('moonTalk');
...
function showMoonMessage(message, duration = 3000, isSarcastic = false) {
    // Si le message est sarcastique/moqueur
    if (isSarcastic) {
        playSFX('moquerie');
    } else {
        playSFX('bip');
